...
velocity1D states t
    = let (t0,_,_) = take 1 states
          (t1,_,_) = take 1 $ drop 1 states
          dt = t1 - t0
          steps = abs $ round (t / dt)
          (_,_,vn) = take 1 $ drop steps states
      in vn

velocityFTXV :: TimeStep -> State1D -> [State1D -> Force] -> Time -> Velocity
...
position1D states t
    = let (t0,_,_) = take 1 states
          ...
          dt = t1 - t0
          steps = abs $ round (t / dt)
          (_,xn,_) = take 1 $ drop steps states
      in xn

positionFTXV :: TimeStep -> State1D -> [State1D -> Force] -> Time -> Position
